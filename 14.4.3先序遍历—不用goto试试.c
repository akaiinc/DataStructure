...
stackNode* popStack(stackNode* stackHead, binaryNode* *bN) {//指向指针的指针,*BN是指向图节点的地址，那么bN就是存贮这个地址的地址
	if (stackHead == NULL) {
...
void stackRead(binaryNode* binaryTreeHead) {//利用栈的非递归方式读取二叉树
	binaryNode *p1, *p2;//p2是树节点的地址，要存储该地址就要用&p2
	...
	while (1) {
		while (1)
		{
			...
			p2 = p1->rightnode;
			if (p2 != NULL) {
				stackHead = pushStack(stackHead, p2);
			}
			p1 = p1->leftnode;
			if (p1 == NULL) break;
		}
