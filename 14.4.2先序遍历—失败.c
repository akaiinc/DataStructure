...
Status pushStack(stackNode* stackHead, binaryNode* bN) {//bN为树节点的指针
	stackNode* p;
...
Status popStack(stackNode* stackHead, binaryNode* *bN) {//指向指针的指针，我马寅驰平生第一次用
	if (stackHead == NULL) {
...
loop: {
	printf("%d ", p1->data);
	p2 = p1->rightnode;
	if (p2 != NULL) pushStack(stackHead, p2);
	p1 = p1->leftnode;
	if (p1 != NULL) {
		goto loop;
	}
	...
	{
		if (popStack(stackHead, &p1) == 0) goto end;
		else {
			goto loop;
		}
	}
